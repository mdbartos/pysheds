...
class RegularViewFinder(BaseViewFinder):
    def __init__(
        ...
        super().__init__(
            ...
            nodata=nodata,
            crs=crs,
            y_coord_ix=y_coord_ix,
            x_coord_ix=x_coord_ix,
        )
    ...
    def move_window(self, dxmin, dymin, dxmax, dymax):
        ...
        new_bbox = (
            xmin_old + dxmin * cell_width,
            ymin_old + dymin * cell_height,
            xmax_old + dxmax * cell_width,
            ymax_old + dymax * cell_height,
        )
        new_shape = (nrows_old + dymax - dymin, ncols_old + dxmax - dxmin)
        new_mask = np.ones(new_shape).astype(bool)
        mask_values = self._mask[
            max(dymin, 0) : min(nrows_old + dymax, nrows_old),
            max(dxmin, 0) : min(ncols_old + dxmax, ncols_old),
        ]
        new_mask[
            max(0, dymax) : max(0, dymax) + mask_values.shape[0],
            max(0, -dxmin) : max(0, -dxmin) + mask_values.shape[1],
        ] = mask_values
        self.bbox = new_bbox
...
class RegularGridViewer:
    ...
    def _view_affine(cls, data, data_view, target_view, x_tolerance=1e-3, y_tolerance=1e-3):
        ...
        y_passed = (
            (np.abs(y_ix - target_row_ix) < y_tolerance) & (y_ix < data_view.shape[0]) & (y_ix >= 0)
        )
        x_passed = (
            (np.abs(x_ix - target_col_ix) < x_tolerance) & (x_ix < data_view.shape[1]) & (x_ix >= 0)
        )
        view[np.ix_(y_passed, x_passed)] = data[y_ix[y_passed]][:, x_ix[x_passed]]
        return view

    @classmethod
    def _view_rectbivariate(
        cls,
        data,
        data_view,
        target_view,
        ...
        viewrows, viewcols = target_view.grid_indices(col_ascending=True, row_ascending=True)
        rows, cols = data_view.grid_indices(col_ascending=True, row_ascending=True)
        viewrows += target_dy
        viewcols += target_dx
        rows += data_dy
        cols += data_dx
        ...
        rbs_interpolator = interpolate.RectBivariateSpline(
            rows[row_bool],
            cols[col_bool],
    ...
    @classmethod
    def _view_rectspherebivariate(
        cls,
        data,
        data_view,
        target_view,
        coords_in_radians=False,
        kx=3,
        ky=3,
        s=0,
        x_tolerance=1e-3,
        ...
        rsbs_interpolator = interpolate.RectBivariateSpline(
            rows[row_bool],
            cols[col_bool],
            data[np.ix_(row_bool[::-1], col_bool)],
            kx=kx,
