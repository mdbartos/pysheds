...
class RFSM:
    ...
    def find_depressions(self):
        ...
        for _ in range(self.max_levels):
            ...
            if self.min_size:
                holes_mask = skimage.morphology.remove_small_objects(
                    holes != 0, min_size=self.min_size
                )
                holes = np.where(holes_mask, holes, 0)
            mask = np.where(holes == 0, holes.min(), holes)
            seed = np.where(holes == 0, mask, holes.max())
    ...
    def find_hierarchy(self):
        ...
        for index in range(len(self.levels) - 1):
            labelmap = (
                pd.Series(
                    self.levels[index + 1][self.levels[index] != 0],
    ...
    def find_watersheds(self):
        ...
        for index in range(len(self.levels) - 1):
            ...
            w = skimage.morphology.watershed(
                self.dem, self.levels[index], mask=mask, watershed_line=True
            )
            w = np.where(mask, w, -1)
            ws.append(w)
    ...
    def find_connections(self):
        ...
        for index in range(len(self.levels) - 1):
            ...
            neighbors = self.ws[index].flat[
                Grid._select_surround_ravel(self, comm, self.ws[index].shape)
            ]
    ...
    def find_drop(self):
        ...
        drop = skimage.morphology.watershed(
            self.dem, np.where(self.ws[0] > 0, self.ws[0], 0), mask=self.ws[1] > 0
        )
        for index in range(1, len(self.ws) - 1):
            num_lower = sum(self.ns[:index])
            base = drop + np.where(
                (self.ws[index] > 0) & (drop == 0), num_lower + self.ws[index], 0
            )
            drop = skimage.morphology.watershed(self.dem, base, mask=self.ws[index + 1] > 0)
        ...
        for index in range(len(self.levels) - 1):
            ...
            if g:
                for j in g:
                    ...
                    parent.elev = np.asscalar(self.dem[self.levels[index + 1] == upper_label].min())
                    child.parent = parent
                    parent.l = child
    ...
    def compute_runoff(self, input_vol):
        v = (
            pd.DataFrame(
                np.column_stack(
                    [
                        self.dropmap[0].loc[self.drop.ravel()].values,
    ...
    def initialize_volumes(self, runoff):
        for level, name, vol in zip(runoff[0].values, runoff[1].values, runoff[2].values):
    ...
    def compute_vol(self, z, node, target_vol):
        ...
        if node.name:
            ...
            boundary = list(
                chain.from_iterable(
                    [self.b[node.level].setdefault(pair, []) for pair in combinations(leaves, 2)]
                )
            )
        ...
        if node.parent:
            if node.name:
                ...
                boundary = list(
                    chain.from_iterable(
                        [
                            self.b[node.level].setdefault(pair, [])
    ...
    def volume_to_level(self, node, waterlevel):
        if node.current_vol > 0:
            ...
            elev = optimize.bisect(self.compute_vol, minelev, maxelev, args=(node, target_vol))
            if node.name:
                mask = self.ws[node.level] == node.name
            else:
                leaves = []
