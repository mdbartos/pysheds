...
class sGrid:
    ...
    def _mfd_accumulation(
        ...
        **kwargs,
        ...
        if efficiency is None:
            if algorithm.lower() == "iterative":
                acc = _self._mfd_accumulation_iter_numba(acc, endnodes, props, indegree, startnodes)
            elif algorithm.lower() == "recursive":
    ...
    def extract_river_network(
        ...
        **kwargs,
        ...
        startnodes = startnodes[(indegree == 0)]
        if algorithm.lower() == "iterative":
            profiles = _self._d8_stream_network_iter_numba(
                endnodes, indegree, orig_indegree, startnodes
            )
        elif algorithm.lower() == "recursive":
            profiles = _self._d8_stream_network_recur_numba(
                endnodes, indegree, orig_indegree, startnodes
            )
        else:
            raise ValueError("Algorithm must be `iterative` or `recursive`.")
    ...
    def extract_profiles(
        ...
        **kwargs,
        ...
        orig_indegree = np.copy(indegree)
        startnodes = startnodes[(indegree == 0)]
        profiles, connections = _self._d8_stream_connection_iter_numba(
        ...
        connections = {
            indices[key]: indices.setdefault(value, indices[key])
    ...
    def cell_dh(
        ...
        **kwargs,
        ...
        if routing.lower() == "d8":
            ...
            dh = self._dinf_cell_dh(dem=dem, fdir=fdir, dirmap=dirmap, nodata_out=nodata_out)
        elif routing.lower() == "mfd":
            dh = self._mfd_cell_dh(dem=dem, fdir=fdir, dirmap=dirmap, nodata_out=nodata_out)
        return dh

    def _d8_cell_dh(self, dem, fdir, dirmap=(64, 128, 1, 2, 4, 8, 16, 32), nodata_out=np.nan):
        # Find nodata cells and invalid cells
        nodata_cells = self._get_nodata_cells(fdir)
        invalid_cells = ~np.isin(fdir.ravel(), dirmap).reshape(fdir.shape)
        # Set nodata cells to zero
    ...
    def _dinf_cell_dh(self, dem, fdir, dirmap=(64, 128, 1, 2, 4, 8, 16, 32), nodata_out=np.nan):
        # Get nodata cells
        nodata_cells = self._get_nodata_cells(fdir)
        # Split dinf flowdir
        fdir_0, fdir_1, prop_0, prop_1 = _self._angle_to_d8_numba(fdir, dirmap, nodata_cells)
    ...
    def _mfd_cell_dh(self, dem, fdir, dirmap=(64, 128, 1, 2, 4, 8, 16, 32), nodata_out=np.nan):
        # Find nodata cells and invalid cells
        nodata_cells = self._get_nodata_cells(fdir)
        # Set nodata cells to zero
        fdir[nodata_cells] = np.int64(0)
    ...
    def cell_distances(
        ...
        **kwargs,
        ...
        fdir = self._input_handler(fdir, **kwargs)
        if routing.lower() == "d8":
            cdist = self._d8_cell_distances(fdir=fdir, dirmap=dirmap, nodata_out=nodata_out)
        elif routing.lower() == "dinf":
            cdist = self._dinf_cell_distances(fdir=fdir, dirmap=dirmap, nodata_out=nodata_out)
        elif routing.lower() == "mfd":
            cdist = self._mfd_cell_distances(fdir=fdir, dirmap=dirmap, nodata_out=nodata_out)
        return cdist

    def _d8_cell_distances(self, fdir, dirmap=(64, 128, 1, 2, 4, 8, 16, 32), nodata_out=np.nan):
        # Find nodata cells and invalid cells
        nodata_cells = self._get_nodata_cells(fdir)
        invalid_cells = ~np.isin(fdir.ravel(), dirmap).reshape(fdir.shape)
        # Set nodata cells to zero
    ...
    def _dinf_cell_distances(self, fdir, dirmap=(64, 128, 1, 2, 4, 8, 16, 32), nodata_out=np.nan):
        # Get nodata cells
        nodata_cells = self._get_nodata_cells(fdir)
        # Split dinf flowdir
        fdir_0, fdir_1, prop_0, prop_1 = _self._angle_to_d8_numba(fdir, dirmap, nodata_cells)
        ...
        dx = abs(fdir.affine.a)
        dy = abs(fdir.affine.e)
        cdist = _self._dinf_cell_distances_numba(fdir_0, fdir_1, prop_0, prop_1, dirmap, dx, dy)
        cdist = self._output_handler(
            data=cdist,
            viewfinder=fdir.viewfinder,
            metadata=fdir.metadata,
    ...
    def _mfd_cell_distances(self, fdir, dirmap=(64, 128, 1, 2, 4, 8, 16, 32), nodata_out=np.nan):
        # Find nodata cells and invalid cells
        nodata_cells = self._get_nodata_cells(fdir)
        # Set nodata cells to zero
        fdir[nodata_cells] = np.int64(0)
    ...
    def cell_slopes(
        ...
        **kwargs,
        ...
        kwargs.update(dem_overrides)
        dem = self._input_handler(dem, **kwargs)
        dh = self.cell_dh(dem, fdir, dirmap=dirmap, nodata_out=np.nan, routing=routing, **kwargs)
    ...
    def rasterize(
        ...
        viewfinder = ViewFinder(affine=transform, shape=out_shape, nodata=fill, mask=mask, crs=crs)
    ...
    def _input_handler(self, data, **kwargs):
        ...
        dataset = self.view(data, data_view=data.viewfinder, target_view=self.viewfinder, **kwargs)
        return dataset
