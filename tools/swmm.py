...
class SwmmIngester(object):
    ...
    def __init__(
        ...
        self.startnodes, self.endnodes = self.grid._construct_matching(
            self.grid.view(catch).copy(), self.flat_idx, dirmap
        )
    ...
    def generate_evaporation(self, **kwargs):
        evaporation = {"CONSTANT": "0.000000", "DRY_ONLY": "NO"}
        # Manual overrides
        for key, value in kwargs.items():
    ...
    def generate_raingages(self, **kwargs):
        ...
        for key, value in kwargs.items():
            raingages[key] = value
        self.raingages = raingages[
            ["name", "rain_type", "time_interval", "snow_catch", "data_source"]
        ]
    ...
    def generate_conduits(self, **kwargs):
        ...
        conduits["name"] = (
            "J"
            + pd.Series(self.startnodes).astype(str)
            + "_"
            + "J"
            + pd.Series(self.endnodes).astype(str)
        )
    ...
    def generate_xsections(self, **kwargs):
        ...
        self.xsections = xsections[
            ["link", "shape", "geom_1", "geom_2", "geom_3", "geom_4", "barrels"]
        ]
    ...
    def generate_timeseries(self, **kwargs):
        ...
        timeseries["date"] = ""
        timeseries["time"] = pd.Series(
            pd.date_range("20140705", periods=timelen, freq="5min").strftime("%H:%M")
        )
    ...
    def generate_storage_uncontrolled(self, ixes, **kwargs):
        ...
        depths = 4
        init_depths = 0.1
        storage_ends = [np.asscalar(self.endnodes[np.where(self.startnodes == ix)]) for ix in ixes]
        ...
        self.storage_uncontrolled = storage_uncontrolled[
            ["name", "elev", "ymax", "y0", "Acurve", "A1", "A2", "A0"]
        ]
    ...
    def generate_storage_controlled(self, ixes, **kwargs):
        ...
        self.storage_controlled = storage_controlled[
            ["name", "elev", "ymax", "y0", "Acurve", "A1", "A2", "A0"]
        ]
    ...
    def generate_control_points(self, ixes, **kwargs):
        for ix in ixes:
            ...
            orif = pd.Series(
                [
                    "{0}_{1}".format(storage_node, downstream_node),
                    storage_node,
                    downstream_node,
                    "BOTTOM",
                    0,
            ...
            orif2 = pd.Series(
                [
                    "{0}_{1}".format(storage_node, storage_ctrl_node),
                    storage_node,
                    storage_ctrl_node,
                    "BOTTOM",
                    0,
            ...
            orif3 = pd.Series(
                [
                    "{0}_{1}".format(storage_ctrl_node, downstream_node),
                    storage_ctrl_node,
                    downstream_node,
                    "BOTTOM",
                    0,
...
if __name__ == "__main__":
    ...
    link_flows.index = (
        pd.Series(link_flows.index).str.split("link_").str[-1].str.split("_Flow_rate").str[0].values
    )
    node_depths.index = (
        pd.Series(node_depths.index)
        .str.split("node_")
        .str[-1]
        .str.split("_Depth_above_invert")
        .str[0]
        .values
    )
    swmm = SwmmIngester(
        grid, "dem", "dir", "catch", "acc", projection, control=False, initialize=True
    )
